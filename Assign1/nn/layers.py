...
class BiRNN(Layer):
    ...
    def forward(self, input):
        ...
        forward  = self.forward_rnn. forward(input)
        backward = self.backward_rnn.forward(self._reverse_temporal_data(input, mask))
        backward = self._reverse_temporal_data(backward, mask)

        output = np.concatenate((forward, backward), axis = 2)
        #####################################################################################
